...
local menu = CreateFrame('frame', 'foglightmenu', WorldMapFrame, 'UIDropDownMenuTemplate')
menu:SetPoint('BOTTOMRIGHT', WorldMapDetailFrame, 'BOTTOMRIGHT', 19, -6)
...
local OverlayInfo = { -- Generated from misc.mpq/WorldMapOverlay.dbc on Feb. 12/13 by Semlar
	...
	if info then
		...
		if not isMicroDungeon then
			...
			for i, item in pairs(info) do
				local textureName, textureWidth, textureHeight, offsetX, offsetY = item:match("^([^/]+)/(%d+)/(%d+)/(%d+)/(%d+)$")
