...
local GetDistanceSqToQuest = GetDistanceSqToQuest
local HasExtraActionBar = HasExtraActionBar
local C_Timer = C_Timer
local GetItemCount = GetItemCount
local RegisterStateDriver = RegisterStateDriver
local SetMapToCurrentZone = SetMapToCurrentZone
local IsQuestBounty = IsQuestBounty
local IsQuestTask = IsQuestTask
local GetQuestTagInfo = GetQuestTagInfo
local GetQuestLogTitle = GetQuestLogTitle
local GetNumQuestLogEntries = GetNumQuestLogEntries

--Global variables that we don't cache, list them here for the mikk's Find Globals script
-- GLOBALS: ExtraActionButton1, GameTooltip, TOOLTIP_UPDATE_TIME, RANGE_INDICATOR, WorldMapFrame
	...
	for index = 1, GetNumWorldQuestWatches() do
		...
		if(questID) then
			...
			if(link) then
				...
				if(not areaID) then
					areaID = itemAreas[tonumber(string.match(link, "item:(%d+)"))]
				end
