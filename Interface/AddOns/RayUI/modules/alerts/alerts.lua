...
local AchievementFrame_SelectAchievement = AchievementFrame_SelectAchievement
...
-- GLOBALS: BLIZZARD_STORE_PURCHASE_COMPLETE, BonusRollFrame, UPGRADED_RECIPE_LEARNED_TITLE
...
local LEARN_TRANSMOG_PATTERN = string.gsub(ERR_LEARN_TRANSMOG_S , "%%s", "(.+)")
local REVOKE_TRANSMOG_PATTERN = string.gsub(ERR_REVOKE_TRANSMOG_S, "%%s", "(.+)")

function AL:CHAT_MSG_SYSTEM(event, message)
	...
	if sourceID then
		...
		local _, _, _, icon, _, _ =C_TransmogCollection.GetAppearanceSourceInfo(sourceID)
...
local function EnableTransmogToasts()
	if CFG.transmog_enabled then
		AL:RegisterEvent("CHAT_MSG_SYSTEM")
	end
end

local function DisableTransmogToasts()
	AL:UnregisterEvent("CHAT_MSG_SYSTEM")
end
...
local function SpawnTestWorldEventToast()
	...
	for _, info in pairs(taskInfo) do
		...
		if QuestMapFrame_IsQuestWorldQuest(questID) and HaveQuestData(questID) then
...
local function SpawnTestTransmogToast()
	...
	local _, _, _, _, _, _, transmogLink = C_TransmogCollection.GetAppearanceSourceInfo(source.sourceID)

	if transmogLink then
		AL:CHAT_MSG_SYSTEM("", string.format(ERR_LEARN_TRANSMOG_S, transmogLink))
	end
end
