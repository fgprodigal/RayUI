...
local function LoadSkin()
	...
	for index, infoBar in pairs(infoBars) do
		...
		if index == 1 then
            ...
            infoBar.ActualHealthBar:SetGradient("VERTICAL", R:GetGradientColor(0, 1, 0))
            ...
            infoBar.ActualHealthBar:SetGradient("VERTICAL", R:GetGradientColor(1, 0, 0))
	...
	hooksecurefunc("PetBattleWeatherFrame_Update", function(self)
		...
		if weather then
			...
			self:ClearAllPoints()
			self:SetPoint("TOP", R.UIParent, 0, -15)
		end
	end)
	...
	local function SkinPetTooltip(tt)
		...
		if tt.ActualHealthBar then
            ...
            tt.ActualHealthBar:SetGradient("VERTICAL", R:GetGradientColor(0, 1, 0))
		end
		if tt.XPBar then
            local r, g, b = tt.XPBar:GetVertexColor()
			tt.XPBar:SetTexture(R["media"].normal)
            tt.XPBar:SetGradient("VERTICAL", R:GetGradientColor(r, g, b))
		end
	...
	hooksecurefunc("PetBattleAbilityTooltip_Show", function()
		...
		t:ClearAllPoints()
		t:SetPoint("TOPRIGHT", R.UIParent, "TOPRIGHT", -4, -4)
	end)
	...
	bar:SetPoint("BOTTOM", R.UIParent, "BOTTOM", 0, 30)
	bar:SetFrameLevel(0)
	...
	bf.TurnTimer:SetPoint("TOP", R.UIParent, "TOP", 0, -140)
    ...
    local xpBarbg = CreateFrame("Frame", nil, bf)
    xpBarbg:SetAllPoints(bf.xpBar)
	xpBarbg:CreateShadow("Background")
    xpBarbg:SetFrameStrata("BACKGROUND")
    xpBarbg:SetFrameLevel(1)
	bf.xpBar:ClearAllPoints()
	bf.xpBar:SetPoint("BOTTOM", bf.TurnTimer.SkipButton, "TOP", 0, 3)
	bf.xpBar:SetScript("OnShow", function(self) self:StripTextures() self:SetStatusBarTexture(R["media"].normal) R:SetStatusBarGradient(self) end)
