...
local function LoadSkin()
    ...
    local function SkinLibDropDownList(level, index)
    ...
    if Lib_UIDropDownMenu_InitializeHelper then
        hooksecurefunc("Lib_UIDropDownMenu_InitializeHelper", SkinLibDropDownList)
    end
    ...
    local function SkinDropDownCheckBox(level, _, dropDownFrame, anchorName, isLib)
        if not level then level = 1 end

        local prefix = isLib and "Lib_" or ""
        local uiScale = UIParent:GetScale()

        local listFrame = _G[prefix.."DropDownList"..level]
        ...
        for j = 1, _G[prefix:upper().."UIDROPDOWNMENU_MAXBUTTONS"] do
            local bu = _G[prefix.."DropDownList"..level.."Button"..j]
            ...
            if bu:IsShown() and x then
                ...
                if not bu.bg then
                    ...
                    _G[prefix.."DropDownList"..level.."Button"..j.."UnCheck"]:SetTexture("")
    ...
    hooksecurefunc("ToggleDropDownMenu", SkinDropDownCheckBox)
    if Lib_ToggleDropDownMenu then
        hooksecurefunc("Lib_ToggleDropDownMenu", function(level, _, dropDownFrame, anchorName) SkinDropDownCheckBox(level, _, dropDownFrame, anchorName, true) end)
    end
