...
function RM:PlayerHasFilteredBuff(frame, db, checkPersonal)
	for buff, value in pairs(db) do
		if value == true then
			...
			local _, icon, _, _, _, _, unitCaster, _, _, _ = AuraUtil.FindAuraByName(name, "player", "HELPFUL")
...
function RM:PlayerHasFilteredDebuff(frame, db)
	for debuff, value in pairs(db) do
		if value == true then
			...
			local _, icon, _, _, _, _, unitCaster, _, _, _ = AuraUtil.FindAuraByName(name, "player", "HARMFUL")
