...
function AB:LoadFakeActionBar()
    local bar1 = CreateFrame("Frame", "RayUIActionBar1", R.UIParent)
    bar1:SetPoint("BOTTOM", R.UIParent, "BOTTOM", 0, 235)
    bar1:SetSize(30, 30)

    local bar3 = CreateFrame("Frame", "RayUIActionBar3", R.UIParent)
    bar3:SetPoint("BOTTOMLEFT", bar1, "BOTTOMRIGHT", AB.db.bar3.buttonspacing, 0)
    bar3:SetSize(30, 30)
end

function AB:Initialize()
    if not self.db.enable then
        self:LoadFakeActionBar()
        return
    end
    ...
    self:RegisterEvent("PLAYER_LOGIN", "CreateCooldown")
