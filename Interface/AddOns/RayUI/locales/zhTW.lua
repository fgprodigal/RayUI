...
do
L["(.*)赢得了：(.+)"] = "(.*)贏得了:(.+)"
L["%s|HRayUILootCollector:%d|h[%s]|h|r %s 赢得了 %s "] = "%s|HRayUILootCollector:%d|h[%s]|h|r %s 贏得了 %s "
L["(.+)选择了(.+)取向：(.+)"] = "(.+)選擇\228?\186?\134?(.+):(.+)"
L["（(.+)）(%d+)点：(.+)（(.+)）"] = "([^-]+) %- (.+)由(.+)擲出(%d+)"
L["（(.+)%+职责加成）(%d+)点：(.+)（(.+)）"] = "%((.+)%+角色加成%)(%d+)點:(.+)%((.+)%)"
L["放弃了："] = "放棄了:"
L["自动放弃了"] = "自動放棄:"
...
L["MoP测试客户端经常崩溃, 这是正常现象"] = "MoP測試客戶端經常崩潰, 這是正常現象"
L["DX9模式下3D头像会出现bug，这是暴雪的问题，所以DX9模式下的3D头像已被禁用"] = "DX9模式下3D頭像會出現bug，這是暴雪的問題，所以DX9模式下的3D頭像已被禁用"
end